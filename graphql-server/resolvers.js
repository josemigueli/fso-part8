  ...
  Query: {
    ...
    allBooks: async (root, args) => {
      ...
      if (args.author && args.genre) {
        ...
        const theBooks = await Book.find({ author: author._id }).populate(
          'author'
        )
      ...
      if (!args.author && args.genre) {
        const byGenres = await Book.find({ genres: args.genre }).populate(
          'author'
        )
        return byGenres
      }
    ...
    addBook: async (root, args, context) => {
      ...
      if (args.author.length < 4) {
        throw new GraphQLError(
          'Author name is shorter than the minimum allowed length (4)',
          {
            extensions: {
              code: 'BAD_USER_INPUT',
      ...
      if (args.title.length < 5) {
        throw new GraphQLError(
          'Book name is shorter than the minimum allowed length (5)',
          {
            extensions: {
              code: 'BAD_USER_INPUT',
      ...
      if (checkBookName) {
        throw new GraphQLError(
          'Book name must be unique and this already exists',
          {
            extensions: {
              code: 'BAD_USER_INPUT',
              invalidArgs: args.title
      ...
      book.author.books = book.author.books.concat(book._id)
